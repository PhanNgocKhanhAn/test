...
def create_pipe():
    ...
    top_pipe = pipe_surface.get_rect(midtop=(700, random_pipe_pos - 650))
...
def check_collision(pipes):
    for pipe in pipes:
        if bird_rect.colliderect(pipe):
            hit_sound.play()
            return False
    if bird_rect.top <= -75 or bird_rect.bottom >= 900:
        return False
    return True
...
pygame.init()
screen = pygame.display.set_mode((576, 1024))   # màn hình rộng hơn
